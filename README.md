...
ロボットは並進速度 $0.2$ [m/s], 角速度 $12\pi/180$ [rad/s]で半時計周りに回転をしている.
<table>
  <tr>
    <td align="center">
...
ランドマークが観測されている間(ピンク色の線が出ている間)は, パーティクルの分布が小さいことが確認できる.  
系統サンプリングを用いたリサンプリングのほうがパーティクルの分布の広がりが大きいことが分かる.
...
|$\sigma_{\ell}$|ランドマークをカメラで観測した際に生じる距離のばらつきの標準偏差|
|$\sigma_{\varphi}$|ランドマークをカメラで観測した際に生じる向きのばらつきの標準偏差|
...
# ライセンスについて
* このソフトウェアパッケージは、MITライセンスの下、再分布および使用が許可されています
。
* © 2026 Shuma Suzuki
